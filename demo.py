...
from CorpusLabeGame import CorpusLabe
import argparse
...
while True:
    ...
    frame = game.draw(frame, body)
    key = renderer.waitKey(frame, delay=1)
    if key == 27 or key == ord('q'):
        break
    #elif key == ord('a'):
    #    game.show_angle = not game.show_angle
    elif key == ord('g'): # either game or acquire angle measures
        game.play_game = not game.play_game
        game.show_angle = not game.show_angle
    #elif key == ord('p'): #
    #    game.current_exercise = abs(game.current_exercise - 1)
    #    game.current_angle[game.current_exercise] = 0
    #    game.current_max_angle[game.current_exercise] = 0
    elif key == ord('r'):
        game.current_max_angle[game.current_exercise] = 0
        game.current_max_angle[abs(game.current_exercise - 1)] = 0
    elif key == ord('h'):
